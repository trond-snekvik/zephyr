		       ...
		       const struct bt_mesh_send_cb *cb, void *cb_data)
{
	...
	if (BT_MESH_ADDR_IS_VIRTUAL(tx->ctx->addr)) {
		ad = bt_mesh_va_label_get(tx->ctx->addr);
	} else {
		ad = NULL;
	}
		    ...
		    struct seg_rx *seg)
{
	...
	BT_DBG("AKF %u AID 0x%02x", akf, aid);
	...
	if (BT_MESH_ADDR_IS_VIRTUAL(rx->ctx.recv_dst)) {
		ad = bt_mesh_va_label_get(rx->ctx.recv_dst);
	} else {
		ad = NULL;
	}

	key = NULL;
	while ((key = bt_mesh_app_key_next(rx, akf, aid, key))) {
		if (seg) {
		...
		if (bt_mesh_app_decrypt(key, akf, aszmic, buf, sdu, ad,
...
static int send_ack(const struct bt_mesh_subnet *sub, uint16_t src,
		    uint16_t dst, uint8_t ttl, uint64_t *seq_auth,
		    uint32_t block, uint8_t obo)
{
	struct bt_mesh_msg_ctx ctx = {
		.net_idx = sub->net_idx,
