...
struct bt_mesh_hb_sub {
	/** Total subscription time in seconds. */
	uint32_t period;
...
struct bt_mesh_hb_cb {
	 ...
	 *
	 * Gets called when the subscription period ends, providing a summary of
	 * the received heartbeat messages.
	 *
	 * @param sub Current Heartbeat subscription parameters.
	 */
...
/** @brief Register a callback for Heartbeat events.
 *
 *  @param cb Callback structure to register. Must point to static memory.
 */
void bt_mesh_hb_add_cb(struct bt_mesh_hb_cb *cb);
